...
def main():
    ...
    parser.add_argument(
        ...
        "--logging_file",
        type=str,
        default="BulkPipelineDownstream.ligandreceptor.log",
    ...
    df_intrxn, df_gene, df_complex, pid2gn = retrieve_cellphonedb(cellphonedb_directory=CELLPHONEDB)
    
    # intake the expression dataframe
    df = pd.read_csv(args.expression_file, index_col=0).fillna(0).astype(float)
    if args.transpose:
        df = df.T
    if args.cpmtransform:
        df = cpm(df)
    ...
    df_profile, p2g = calculate_expression(df_gene=df_gene, df_complex=df_complex, df=df)
